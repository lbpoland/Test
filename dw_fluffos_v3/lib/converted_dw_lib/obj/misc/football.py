# Generated from /mnt/home2/test/Test/dw_fluffos_v3/lib/obj/misc/football.c (2003 FluffOS v3) for Forgotten Realms MUD
...
import asyncio
    ...
    def do_kick(self, in_dir: MudObject) -> int:
            if (!sizeof(in_dir))
            return 0
            if (move(in_dir[0])) {
            await in_dir[0].send(this_player()->query_cap_name()+" kicks "+
            self.query_short()+" at you, but it falls on the floor.\n")
            ...
            await in_dir[0].send(this_player()->query_cap_name()+" kicks a "
